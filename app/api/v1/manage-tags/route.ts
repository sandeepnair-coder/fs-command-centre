...
import { ManageTagsSchema } from "@/lib/api/schemas";
import { manageTaskTags } from "@/lib/services/task-service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = ManageTagsSchema.safeParse(body);
    if (!parsed.success) {
      return apiError("VALIDATION_ERROR", parsed.error.issues.map((i) => `${i.path.join(".")}:\ ${i.message}`).join("; "));
    }
    const result = await manageTaskTags(parsed.data);
