...
import { ManageColumnsSchema } from "@/lib/api/schemas";
import { manageColumnsByAgent } from "@/lib/services/client-service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = ManageColumnsSchema.safeParse(body);
    if (!parsed.success) {
      return apiError("VALIDATION_ERROR", parsed.error.issues.map((i) => `${i.path.join(".")}:\ ${i.message}`).join("; "));
    }
    const result = await manageColumnsByAgent(parsed.data);
