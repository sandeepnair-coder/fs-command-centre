...
import { SearchClientsSchema } from "@/lib/api/schemas";
import { searchClients } from "@/lib/services/client-service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = SearchClientsSchema.safeParse(body);
    if (!parsed.success) {
      return apiError("VALIDATION_ERROR", parsed.error.issues.map((i) => `${i.path.join(".")}:\ ${i.message}`).join("; "));
    }
    const result = await searchClients(parsed.data);
