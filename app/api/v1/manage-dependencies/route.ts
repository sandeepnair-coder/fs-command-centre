...
import { ManageDependenciesSchema } from "@/lib/api/schemas";
import { manageDependencies } from "@/lib/services/task-service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = ManageDependenciesSchema.safeParse(body);
    if (!parsed.success) {
      return apiError("VALIDATION_ERROR", parsed.error.issues.map((i) => `${i.path.join(".")}:\ ${i.message}`).join("; "));
    }
    const result = await manageDependencies(parsed.data);
