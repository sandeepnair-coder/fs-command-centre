...
import { AddClientFactSchema } from "@/lib/api/schemas";
import { addClientFactsByAgent } from "@/lib/services/client-service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = AddClientFactSchema.safeParse(body);
    if (!parsed.success) {
      return apiError("VALIDATION_ERROR", parsed.error.issues.map((i) => `${i.path.join(".")}:\ ${i.message}`).join("; "));
    }
    const result = await addClientFactsByAgent(parsed.data);
