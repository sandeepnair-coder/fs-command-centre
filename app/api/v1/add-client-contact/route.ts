...
import { AddClientContactSchema } from "@/lib/api/schemas";
import { addClientContactByAgent } from "@/lib/services/client-service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = AddClientContactSchema.safeParse(body);
    if (!parsed.success) {
      return apiError("VALIDATION_ERROR", parsed.error.issues.map((i) => `${i.path.join(".")}:\ ${i.message}`).join("; "));
    }
    const result = await addClientContactByAgent(parsed.data);
